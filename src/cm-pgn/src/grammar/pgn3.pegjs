...
header
  = "[" key:headerKey " \"" value:headerValue "\" ]" { return { key: key, value: value }; }
...
pgn
  = headers:header* pw:pgnStartWhite all:pgnBlack?
      { var arr = (all ? all : []); arr.unshift(pw); return headers.concat(arr); }
  / headers:header* pb:pgnStartBlack all:pgnWhite?
    { var arr = (all ? all : []); arr.unshift(pb); return headers.concat(arr); }
  / headers:header* all:pgnBlack?
    { return headers.concat(all); }
  / headers:header* all:pgnWhite?
    { return headers.concat(all); }
  / whiteSpace?
    { return [[]]; }
